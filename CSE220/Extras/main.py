        ...
        if self.head is not None:
            while current_node is not None:
                ...
                while next_node is not None:
                    if current_node.element > next_node.element:
                        temporary = current_node.element
                        current_node.element = next_node.element
                        next_node.element = temporary
...
#Task_2-5
   # def insert(self,newElement):
            ...
            #self.sum+=1
#Task_2-5 and 6
    def insert(self,newElement,index=None):                                                                     #2 tay same e st bhai
        if (index!=None):
            if(self.head==None):
                print("The list is Empty!")
            elif(self.head!=None):
...
#Task_02-7
    def remove(self, deletekey):
        if self.head == None:
            raise Exception("Sorry, empty list not allowed")
        xHead = self.head
        if xHead.element == deletekey:
            xHead = xHead.next
        else:
            while (xHead is not None):
                if xHead.next == None:
                    break
                if xHead.next.element == deletekey:
                    if xHead.next.next == None:
                        xHead.next = None
                    else:
...
class Tester:
    ...
    list1.selection_sort()                                      #Use this  to sort(Sorting from small to big)
