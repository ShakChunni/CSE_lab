...
#print(listx)
#print(len_x[0]) #[8, 8, 4, 6, 4, 6, 4, 10, 6, 8, 8, 6, 6, 6, 6, 6, 8, 6, 6, 10]
#print(len(len_x))
abc = []
for i in range (0, len(len_x)):                         #condtions to check length of each line after strgt_dis and main_city shoraisi (idk me stupid)
    if i == 0:
      if len_x[i] == 2:
          cond = listx[0:0]
          new_start = 0                                   #==2 mane hocche line er length
      if len_x[i] == 4:
          cond = listx[0:1]
          new_start = 1
      elif len_x[i] == 6:
      ...
      if len_x[i] == 4:
        ...
        cond = listx[new_start:new_start+3] #[3:3+3] #[0:3] #0:4    #3+ whatever tui nisish
...
for i in range (len(main_city)):
  straightline_dict[main_city[i]] = straightline_distance[i]      #dictionary of current_city and straightline_distance
...
if __name__ == "__main__":
    ...
    if not optimal_nodes:
      ...
      for i in optimal_nodes:
        print(i, end=" -> ")
      print(" ")
      print("Total Distance:",visited_nodes[len(visited_nodes) - 1][1], "km")