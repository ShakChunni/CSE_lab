...
def alphabeta(depth, state, maximizingPlayer, list, alpha, beta):
    ...
    if maximizingPlayer:  # alpha puring
        best = -math.inf
        for i in range(0, 2):
            # check korsi left side and then compared and right side
            val = alphabeta(depth + 1, state * 2 + i, False, list, alpha, beta)
            best = max(best, val)
...
if __name__ == "__main__":
    ...
    # last 2 digits swapped and multiplied by 1.5
    maximum_point = int(points_needed*1.5)

# task1
    generated_nums = random.sample(
        range(minimum_point, maximum_point), total_nums)
    print("Task 1:")
